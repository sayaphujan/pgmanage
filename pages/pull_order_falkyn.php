...
$request = "https://design.peregrinemfginc.com/do/api_pull_order_falkyn/?id=$id&s=$s";    
...
if (($fp = fopen('file.csv', 'r')) !== FALSE)
{
    ...
    else
    {
        ...
        $project_id = $project['id'];
         ...
         $query = 'UPDATE projects SET `product` = \''.sf($_POST['product']).'\', `notes`   = \''.sf($order_data['Customer_Notes']).'\',  `pod`     = \''.sf($order_data['Production_Cycle']).'\', `estimated_completion` = \''.sf($order_data['Received_Date']).'\', `global_vars` = \''.sf(json_encode($g_vars)).'\', `peregrine_date_modified` = \''.sf($order_data['Last_Update_Data']).'\', `final_design`=\''.sf($order_data['Final_Design']).'\'  WHERE id=\''.sf($project_id).'\'';
         //echo $query;
         mysqli_query($link, $query);
    }
    ...
    for($i=1; $i<=20; $i++)
    {
        ...
        {
            $img_url = 'https://design.peregrinemfginc.com/zip/Falkyn_order_'.$id.'/'.$order_data['EM'.$i.' Logo'];
    ...
    //echo    "<script>
    //       window.location.href = '?page=convert&id=".$project_id."';
    //    </script>";
