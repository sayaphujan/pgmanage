...
if(isset($_GET['add'])) 
{
    if(isset($_POST['name'])) 
    {
        ...
        echo '<META HTTP-EQUIV="Refresh" Content="0; URL=?page=product&id='.mysqli_insert_id($link).'">';
    }
...
if(isset($_POST['name'])) 
{
	...
	if(isset($_POST['steps'])){
	    for($i = 0; $i<count($_POST['steps'])-1; $i++) {
    		...
    		if(isset($_POST['step_variable_id_'.$step_id])){
    		    for($x = 0; $x < count($_POST['step_variable_id_'.$step_id]); $x++) {
                    ...
                    $que = mysqli_query($link, 'SELECT * FROM product_sub_steps WHERE product=\''.sf($_GET['id']).'\' AND s_order=\''.sf($x).'\' AND step=\''.sf($step_id).'\'');
                    if(mysqli_num_rows($que)>0){
                        
                        //echo 'UPDATE product_sub_steps SET s_order=\''.$x.'\', `name`=\''.sf($_POST['step_variable_'.$step_id][$x]).'\', `part`=\''.sf($_POST['step_variable_part_'.$step_id][$x]).'\' WHERE s_order=\''.$x.'\' AND step=\''.$step_id.'\'';
        			    mysqli_query($link, 'UPDATE product_sub_steps SET s_order=\''.sf($x).'\', `name`=\''.sf($_POST['step_variable_'.$step_id][$x]).'\', `part`=\''.sf($_POST['step_variable_part_'.$step_id][$x]).'\' WHERE s_order=\''.sf($x).'\' AND step=\''.sf($step_id).'\'');
                   
                    }else{
                        
                        $_POST['step_variable_part_'.$step_id][$x] = ($_POST['step_variable_part_'.$step_id][$x] == '') ? '0' : $_POST['step_variable_part_'.$step_id][$x];
                        
                        //echo 'INSERT INTO product_sub_steps (`product`, `step`, `type`, `s_order`, `name`, `part`) VALUES (\''.sf($_GET['id']).'\', \''.sf($step_id).'\', \''.sf($_POST['step_variable_type_'.$step_id][$x]).'\', \''.$x.'\', \''.sf($_POST['step_variable_'.$step_id][$x]).'\', \''.sf($_POST['step_variable_part_'.$step_id][$x]).'\')';
                        mysqli_query($link, 'INSERT INTO product_sub_steps (`product`, `step`, `type`, `s_order`, `name`, `part`) VALUES (\''.sf($_GET['id']).'\', \''.sf($step_id).'\', \''.sf($_POST['step_variable_type_'.$step_id][$x]).'\', \''.$x.'\', \''.sf($_POST['step_variable_'.$step_id][$x]).'\', \''.sf($_POST['step_variable_part_'.$step_id][$x]).'\')');
                    }
        		}
    		}
    		
    		if($_POST['step_id'][$i]>0 && $_POST['step_id'][$i]!=='new') {
    			mysqli_query($link, 'UPDATE product_steps SET name=\''.sf($_POST['step_name'][$i]).'\', metadata = \''.sf(json_encode($step_meta)).'\', `order`=\''.sf($i).'\', `email_template`=\''.sf($_POST['email_template'][$i]).'\' WHERE product=\''.sf($_GET['id']).'\' AND id=\''.sf($_POST['step_id'][$i]).'\'');
    		} else {
    		    
    			mysqli_query($link, 'INSERT INTO product_steps (`name`, `product`, `order`, `metadata`, `email_template`) VALUES (\''.sf($_POST['step_name'][$i]).'\', \''.sf($_GET['id']).'\', \''.sf($i).'\', \''.sf(json_encode($step_meta)).'\', \''.sf($_POST['email_template'][$i]).'\')');
    		}
	...
	if(isset($_POST['part']))
	{
	    ...
	    for($i = 0; $i<count($_POST['part']); $i++) 
	    {
    		...
    		if(isset($_POST['part_variable_name_'.$part_id]))
    		{
    		...
    		if (!isset($_POST['batch_lot_required_'.$part_id])) {
                $_POST['batch_lot_required_'.$part_id] = 0;
            } else {
                $_POST['batch_lot_required_'.$part_id] = 1;
            }
    		...
    		mysqli_query($link, 'UPDATE product_parts SET `name`=\''.sf($_POST['part_name_'.$part_id]).'\', `batch_lot`=\''.sf($_POST['batch_lot_required_'.$part_id]).'\', `p_order`=\''.sf($i).'\', `variables`=\''.sf($part_vars).'\' WHERE id=\''.sf($part_id).'\'');			
    	}
	}
	...
	if(isset($_POST['categories'])){
		...
		mysqli_query($link, 'UPDATE product_part_categories SET `name`=\''.sf($_POST['category_name'][$i]).'\', `c_order`=\''.sf($i).'\' WHERE id=\''.sf($cat_id).'\'');
    ...
    if(isset($_POST['global_var_group'])){
    	...
    	foreach($_POST['global_var_group'] as $key=>$g_id) {
    	        ...
    	        if(isset($_POST['var_group_name_'.$g_id])){
...
function documentation($step, $sub_step, $order) {
    global $link;
	$html = '';
	
    $q = mysqli_query($link, 'SELECT * FROM project_step_documents WHERE product=\''.sf($_GET['id']).'\' AND step=\''.sf($step).'\' AND sub_step=\''.sf($sub_step).'\' AND m_order=\''.sf($order).'\'');
    //echo 'SELECT * FROM project_step_documents WHERE product=\''.sf($_GET['id']).'\' AND step=\''.sf($step).'\' AND sub_step=\''.sf($sub_step).'\' AND m_order=\''.sf($order).'\'';
...
function addtext(text) {
    document.product_form.email_template.value += text;
}
...
<div class="tab-content">
  <div id="main_tab" class="tab-pane fade in active">
			  ...
			  <div class="modal-body">
			      <?php
                    ...
                    while($row = mysqli_fetch_array($que)){
                        if($row['Field'] != 'id'){
                            echo '<div class="col-md-3 col-mt-15">';
                            echo '<a href="#" onclick="insertAtCaret(\'email_template\', \'%'.$row['Field'].'\');return false;">'.$row['Field'].'</a>&nbsp;&nbsp;';
                            echo '</div>';
                        }
                    }
						...
						echo '<br/><div class="step_input" id="sub_step_'.$sub_step['id'].'">
							...
							<div>
								...
								<div class="col-md-2">
									...
									<button type="button" class="btn" onclick="step_documentation('.$step['id'].', '.$sub_step['id'].', '.$x.');">Documentation</button>
								</div>
								<div class="col-md-4">
								    <label class="control-label">&nbsp;</label>
									<br>
									'.documentation($step['id'], $sub_step['id'], $x).'
								</div>
							</div>
							<div class="clear"></div>
			...
			<input type="hidden" name="global_var_group[]" value="<?=$g_id?>">
				<h4>Group <input type="text" name="var_group_name_<?=$g_id?>" value="<?=$group['name']?>">
					<small>
						...
						<button class="btn btn-danger" type="button" onclick="remove_global_var_group(<?=$g_id?>, '<?=$group['name']?>')">Remove Category</button>
					</small>
				</h4>
	...
	<div id="parts_tab" class="tab-pane fade">
		...
		function step_documentation(id, sub, order) {
			$.get( "exec/product_options/?option=step_documentation&product=<?=$_GET['id']?>&step="+id+"&sub_step="+sub+"&order="+order, function( data ) {
      ...
      save_product()
		...
		function remove_global_var_group(id, name) {
		    $.get( "exec/product_options/?option=remove_global_var_group&id=<?=$_GET['id']?>&name="+name, function( data ) {
        ...
        $.post('<?=root()?>exec/product/?id=<?=$_GET['id']?>', $('#product_form').serialize(), function(){
            $.notify('Product have been successfully saved', 'success');
        });
		}
